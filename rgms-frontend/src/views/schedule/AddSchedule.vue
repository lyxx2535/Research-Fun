...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        ddl: [{ required: true, message: "提醒日期不能为空", trigger: "blur" }],
      },
      memberList: [],
      targetList: [],
      //禁止选择当前日期之前的日期
      ...
      pickerOptionsDdl: {
        disabledDate: (time) => {
          if (this.ruleForm.date) {
            return (
              time.getTime() > new Date(this.ruleForm.date).getTime() ||
  ...
  methods: {
    ...
    generateMemberList() {
      this.$axios
        .get("/researchGroup/detailedInfo?groupId=" + this.groupId)
        .then((res) => {
          let originList = res.data.data.studentInfos;
          if (originList.length !== 0) {
            originList.forEach((member, index) => {
              if (member.truename == undefined) {
                ...
                this.memberList.push({
                  label: member.username + "(" + member.truename + ")",
                  key: member.userId,
                });
              }
            });
          }
          originList = res.data.data.teacherInfos;
          if (originList.length !== 0) {
            originList.forEach((member, index) => {
              if (member.truename == undefined) {
                this.memberList.push({
      ...
      this.$refs.ruleFormRef.validate((valid) => {
        if (valid) {
          ...
          if (this.userType === "TEACHER") {
            ...
            this.$axios
              .post("/schedule/createForTeacher", teacherSchedule)
              .then((res) => {
                if (res.data !== "") {
                  this.$message.error({
          ...
          //学生创建日程
            ...
            this.$axios
              .post("/schedule/create?userId=" + this.id, this.ruleForm)
              .then(
                (res) => {
                  if (res.data === "") {
                    this.$message.success({
